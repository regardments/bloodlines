...
return (function()
	...
	function Library:MakeDraggable(Frame, Cutoff)
		...
		Frame.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 then
				if Library.SearchBoxPressed then
					Library.SearchBoxPressed = nil
					return
				end
	...
	function Library:RegisterSearchItem(Text, Tab, Frame, Scroller, ElementType)
		...
		table.insert(Library.SearchItems, {
			Text = Text,
			Tab = Tab,
			Frame = Frame,
			Scroller = Scroller,
	...
	function Library:FlashElement(Frame)
		if not Frame or not Frame.Parent then
			return
		end

		local OriginalColor = Frame.BorderColor3
		...
		task.spawn(function()
			for _ = 1, 2 do
				...
				Frame.BorderColor3 = OriginalColor
				task.wait(0.09)
			end
		end)
	end
	...
	do
		...
		function Funcs:AddLabel(Text, DoesWrap)
			...
			Library:RegisterSearchItem(Text, Groupbox.Tab, TextLabel, Groupbox.Scroller, "Label")
		...
		function Funcs:AddButton(...)
			...
			Library:RegisterSearchItem(Button.Text, Groupbox.Tab, Button.Outer, Groupbox.Scroller, "Button")
		...
		function Funcs:AddInput(Idx, Info)
			...
			Library:RegisterSearchItem(Info.Text, Groupbox.Tab, TextBoxOuter, Groupbox.Scroller, "Input")
		...
		function Funcs:AddToggle(Idx, Info)
			...
			Library:RegisterSearchItem(Info.Text, Groupbox.Tab, ToggleOuter, Groupbox.Scroller, "Toggle")
		...
		function Funcs:AddSlider(Idx, Info)
			...
			Library:RegisterSearchItem(Info.Text, Groupbox.Tab, SliderOuter, Groupbox.Scroller, "Slider")
		...
		function Funcs:AddDropdown(Idx, Info)
			...
			Library:RegisterSearchItem(Info.Text, Groupbox.Tab, DropdownOuter, Groupbox.Scroller, "Dropdown")
		...
		function Funcs:AddDependencyBox()
			...
			Depbox.Tab = Groupbox.Tab
			Depbox.Scroller = Groupbox.Scroller
	...
	function Library:ManuallyManagedNotify(Text)
		if type(shared.Lycoris) == "table" and shared.Lycoris.silent then
			return
		end
	...
	function Library:CreateWindow(...)
		...
		local SearchBoxOuter = Library:Create("Frame", {
			BackgroundColor3 = Color3.new(0, 0, 0),
			BorderColor3 = Color3.new(0, 0, 0),
			Position = UDim2.new(1, -144, 0, 3),
			Size = UDim2.new(0, 140, 0, 19),
			ZIndex = 3,
			Parent = Inner,
		})

		Library:AddToRegistry(SearchBoxOuter, {
			BorderColor3 = "Black",
		})
		...
		Library:OnHighlight(SearchBoxOuter, SearchBoxOuter, { BorderColor3 = "AccentColor" }, { BorderColor3 = "Black" })

		SearchBoxInner.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 then
				Library.SearchBoxPressed = true
			end
		end)

		local SearchList = Library:Create("Frame", {
			BackgroundColor3 = Color3.new(0, 0, 0),
			BorderColor3 = Color3.new(0, 0, 0),
			ZIndex = 28,
			Visible = false,
		...
		local SearchListInner = Library:Create("Frame", {
			BackgroundColor3 = Library.MainColor,
			BorderColor3 = Library.OutlineColor,
			BorderMode = Enum.BorderMode.Inset,
			Size = UDim2.new(1, 0, 1, 0),
		...
		Library:AddToRegistry(SearchListInner, {
			BackgroundColor3 = "MainColor",
			BorderColor3 = "OutlineColor",
		})

		local SearchListScroll = Library:Create("ScrollingFrame", {
			BackgroundTransparency = 1,
			BorderSizePixel = 0,
			CanvasSize = UDim2.new(0, 0, 0, 0),
			Size = UDim2.new(1, 0, 1, 0),
			ZIndex = 29,
			Parent = SearchListInner,
		...
		Library:AddToRegistry(SearchListScroll, {
			ScrollBarImageColor3 = "AccentColor",
		})
		...
		local function JumpToSearchResult(Item)
			...
			if not Item or not Item.Tab then
				return
			end
			...
			local Scroller = Item.Scroller

			if Frame and Frame.Parent then
				local Ancestor = Frame.Parent

				while Ancestor and Ancestor ~= Scroller and Ancestor:IsA("Frame") do
				...
				if Scroller and Frame.Parent then
					local Y = Frame.AbsolutePosition.Y - Scroller.AbsolutePosition.Y + Scroller.CanvasPosition.Y
					Scroller.CanvasPosition = Vector2.new(Scroller.CanvasPosition.X, math.max(0, Y))
				end

				Library:FlashElement(Frame)
			end
		end

		local function RebuildSearchResults(Query)
			for _, Child in next, SearchListScroll:GetChildren() do
			...
			for _, Item in next, Library.SearchItems do
				...
				if Item.Text:lower():find(LowerQuery, 1, true) then
					Count = Count + 1
					...
					local RowLabel = Library:CreateLabel({
						...
						Position = UDim2.new(0, 6, 0, 0),
						TextSize = 12,
						Text = TabName and string.format("%s  (%s)", Item.Text, TabName) or Item.Text,
						TextXAlignment = Enum.TextXAlignment.Left,
						ZIndex = 31,
						Parent = Row,
					})
					...
					Library:OnHighlight(
						Row,
						Row,
						{ BorderColor3 = "AccentColor", ZIndex = 31 },
						{ BorderColor3 = "OutlineColor", ZIndex = 30 }
					)
			...
			local ListHeight = math.clamp(Count * 18 + 2, 0, (8 * 18) + 2)

			SearchList.Size = UDim2.fromOffset(SearchBox.AbsoluteSize.X, ListHeight)
			SearchList.Position = UDim2.fromOffset(
				SearchBox.AbsolutePosition.X,
				SearchBox.AbsolutePosition.Y + SearchBox.AbsoluteSize.Y + 1
			)
			SearchListScroll.CanvasSize = UDim2.fromOffset(0, math.max(ListHeight - 2, 0))

			SearchList.Visible = Count > 0
		end
		...
		SearchBox.FocusLost:Connect(function(Enter)
			...
			if Enter and SearchResults[1] then
				JumpToSearchResult(SearchResults[1])
			end
		end)
		...
		TabListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			TabArea.CanvasSize = UDim2.new(0, TabListLayout.AbsoluteContentSize.X, 0, 21)
		end)
		...
		function Window:AddTab(Name)
			...
			function Tab:AddGroupbox(Info)
				...
				Groupbox.Tab = Tab
				Groupbox.Scroller = Info.Side == 1 and LeftSide or RightSide
